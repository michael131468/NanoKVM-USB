...
## MacOS Mouse Side-Button Support (optional)

On macOS the app can optionally forward mouse side buttons to the KVM target
as `BTN_SIDE`/`BTN_EXTRA` HID reports, driven by a native CGEventTap addon in
`native/macos/`. The app runs fine without this — if the addon isn't built,
side-button forwarding is silently disabled and everything else works
normally.
